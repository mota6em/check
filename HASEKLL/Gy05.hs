...
login = do
  currentUser <- ask
  users <- get
  if elem currentUser users then
    tell ["A " ++ currentUser ++ " nevu felhasznalo sikeresen belepett."]
  else
    throwError "Nem letezo felhasznalo!"

-- Try login as function
tryLoginAs :: (Logintype m) => String -> m ()
tryLoginAs username = local (const username) (login `catchError` \err -> tell [err])

-- Example usage with the combined monads
type App a = ExceptT String (WriterT [String] (ReaderT String (State [String]))) a

runApp :: App a -> String -> [String] -> ((Either String a, [String]), [String])
runApp app currentUser initialState =
  runState (runReaderT (runWriterT (runExceptT app)) currentUser) initialState

-- Example main to demonstrate the usage
main :: IO ()
main = do
  let initialState = []
      ...
      app = do
        ...
        tryLoginAs "testUser"
        tryLoginAs "unknownUser"
      ((result, log), finalState) = runApp app currentUser initialState
  print result
  mapM_ putStrLn log
  print finalState