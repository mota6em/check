...
foldrList f b Nil = b 
foldrList f b (Cons a x) = f a (foldrList f b x)
...
instance Foldable Tree where 
  ...
  foldr f b (Node x a xx) = (foldr f (f a (foldr f b xx)) x)  
...
instance Foldable f => Foldable (Apply f) where
  foldr f b (MkApply foldable_e) = foldr f b foldable_e
...
instance Foldable SkipList where
    -- foldr :: (a -> b -> b) -> b -> SkipList a -> SkipList b
    foldr f b (Skip _) = b 
    foldr f b (SCons a ls) = f a (foldr f b ls)
    foldr f b SNill = b
...
instance Foldable CrazyType where
    ...
    foldr f b (C2 a n) = f a b 
    foldr f b (C3 ls) = foldr f b ls
