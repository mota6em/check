...
instance Alternative Parser where
  ...
  (<|>) pa pr = Parser $ \s -> case runParser pa s of 
    Nothing -> runParser pr s 
    Just r -> Just r
...
-- alm(a|ák)
p1 :: Parser ()
p1= string "alm" >> (char 'a' <|> string "ak")

-- c(i+)ca
p2 :: Parser ()
p2 = string "c" >> optional (string "i+") >> string "ca"
...
-- (alma|banana)?
p4 :: Parser ()
p4 = () <$ some (string "alma" <|> string "banana")

-- [A-Z]{10}
p5 :: Parser ()
p5 = () <$ replicateM 10 (satisfy (`elem` ['A'..'Z']))

-- \d{2}
p6 :: Parser ()
p6 = () <$ replicateM 2 (digitToInt <$> satisfy isDigit)
