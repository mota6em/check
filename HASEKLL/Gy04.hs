...
-- magicFunctionM x = (>>=) (incrementIfEven x) (\z -> (combineThrees (*) x z)) -- >>= (\r -> return r)
magicFunctionM a = (>>=) (incrementIfEven a) (\z -> (combineThrees (*) a z)) >>= (\r -> return r)
-- Így lehet több olyan műveletet komponálni, amelyeknek vannak mellékhatásaik
-- Akinek nem tetszik a >>= irogatás létezik az imperatív stílusú do notációa
{-
do
   x <- y
   a
...
readAndConcat' :: IO ()
readAndConcat' = (>>=) getLine (\x -> (>>=) getLine (\r -> putStrLn (x ++ r)))
...
takeFromSt n = do 
  xs <- get 
  let ys = take (fromIntegral n) xs 
  let zs = drop (fromIntegral n) xs 
  put zs 
  return ys
