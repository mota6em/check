...
instance Traversable NonEmpty where
  ...
  traverse f (NECons a noe) = NECons <$> f a <*> traverse f noe
instance Traversable NonEmpty2 where
  traverse :: (Applicative f) => (a-> f b) -> NonEmpty2 a -> f (NonEmpty2 b)
  traverse f (NECons2 a lis) = NECons2 <$> f a <*> traverse f lis
...
instance Traversable (TriEither fixed1 fixed2) where
  traverse :: (Applicative f) => (a->f b) -> TriEither fi1 fi2 a -> f (TriEither fi1 fi2 b)
...
instance Traversable (BiList fixed) where
  ...
  traverse f (ACons a lis) =ACons <$> pure a <*> traverse f lis 
  traverse f (BCons a lis) = BCons <$> f a <*> traverse f lis 
