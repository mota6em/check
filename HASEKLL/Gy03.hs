...
foldrQuintuple f b (Quintuple a a' a'' a''' a'''') = f a (f a' (f a'' (f a''' (f a'''' b))))

foldrList :: (a -> b -> b) -> b -> List a -> b
foldrList f b Nil = b 
foldrList f b (Cons a lis) = f a (foldrList f b lis)
...
instance Foldable (BiList fixed) where
  foldr f b ABNill = b 
  foldr f b (ACons a blis) = foldr f b blis 
  foldr f b (BCons a flis) = f a (foldr f b flis) 
-- Magasabbrendű megkötések
instance Foldable f => Foldable (Apply f) where
  -- foldr f b (MkApply fex) = f b fex 
...
instance Foldable Tree2 where 
  foldr f b Leaf2 = b 
  foldr f b (Node2 left a right) = f a (foldr f (foldr f b left) right)
...
instance Foldable Tree3 where 
  ...
  foldr f b (Node3 left right) = foldr f (foldr f b left) right
